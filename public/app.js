...
function renderModal(order) {
  ...
  const musicFrameLineItems = (order.line_items || []).filter(li => /muziek-?frame|valentijn-?frame/i.test(li.title || ''));
  ...
  const autoFrameLineItems = (order.line_items || []).filter(li => /auto-?frame/i.test(li.title || ''));
