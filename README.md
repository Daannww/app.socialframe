...
weergegeven. Oorzaak gevonden in `drawMixedText` (`server/pdf-shared.js`,
gedeeld door muziekframe/auto-frame/sound-frame/kentekenplaathouder): een
emoji werd daar altijd getekend op 1.15x de puntgrootte van de lopende
tekst — een vaste aanname die bedoeld was om een emoji "even zwaar" te
laten ogen als tekst. Bij kentekenplaathouders (vaak korte, volledig
hoofdlettertekst) is de werkelijke hoogte van een hoofdletter echter maar
zo'n 0,70-0,81x de puntgrootte (afhankelijk van het lettertype) — dus die
vaste 1.15x viel daar duidelijk uit de toon: de emoji werd systematisch
groter dan de bestelde tekst zelf, niet incidenteel.

**Fix**: een nieuwe functie `meetGrootsteLetterHoogtePt` in
`pdf-shared.js` meet de WERKELIJKE contour (niet een aangenomen
capHeight-verhouding) van elk teken dat daadwerkelijk in de bestelde tekst
voorkomt — via het onderliggende fontkit-font-object dat pdf-lib zelf ook
gebruikt om tekst te coderen (`font.embedder.font`), zodat ook een accent
(bv. "É") of een onderlengte (bv. een "J" die net onder de basislijn
uitsteekt) meetellen. `drawMixedText` heeft een nieuwe, optionele
`opts.maxEmojiSizePt`-parameter gekregen: de emoji wordt dan getekend op
`min(1.15 × puntgrootte, opts.maxEmojiSizePt)`, met dezelfde relatieve
"overhang" onder de basislijn als voorheen, alleen geschaald naar de
(mogelijk kleinere) uiteindelijke grootte. Alleen
`server/kentekenplaathouder.js` geeft deze nieuwe parameter mee (gemeten
over de specifieke, per order bestelde tekst); muziekframe/auto-frame/
sound-frame roepen `drawMixedText` ongewijzigd aan (geen `opts`) en blijven
dus exact het oude, al bevestigde 1.15x-gedrag gebruiken — bewust niet in
één keer overal aangepast, om niets te riskeren aan al goedgekeurd gedrag
elders.

Bijzonderheid ontdekt tijdens het testen: een kleine letter kan soms
GROTER meten dan een hoofdletter — bv. de "i" in Montserrat Bold heeft een
stipje dat net boven de capHeight uitsteekt (bbox-top 787 t.o.v. 700 voor
de hoofdletters). Dat is geen bug maar precies de bedoeling: de functie
meet de WERKELIJKE grootste letter van de daadwerkelijk bestelde tekst,
in plaats van een vaste aanname — dus ook zulke font-eigenaardigheden
worden correct meegenomen.

**Getest**:
- Losstaand testje dat `page.drawImage` onderschept om de daadwerkelijk
  doorgegeven emoji-afmeting te controleren: bevestigd dat een aanroep
  ZONDER de nieuwe `opts` (dus alle andere producten) nog steeds exact de
  oude 1.15x-grootte geeft (geen enkel verschil), en dat een aanroep MET
  `opts.maxEmojiSizePt` de emoji nooit groter tekent dan die grens.
- Bevestigd dat tekst die uitsluitend uit emoji bestaat (geen "letters" om
  aan te meten) een `null`-grens oplevert, en dat `drawMixedText` daarmee
  keurig terugvalt op het oude 1.15x-gedrag i.p.v. de emoji op grootte 0
  te zetten of te crashen.
- De daadwerkelijke `generateKentekenplaathouderPdf`-functie succesvol
  getest met meerdere lettertypes (Montserrat, Oswald, de Helvetica-Bold-
  noodgreep bij een onbekend lettertype), met hoofdletters, kleine letters
  en tekst die alleen uit emoji bestaat — geen enkele fout.
- Volledige regressietest: alle servermodules laden nog zonder fouten en
  zijn syntax-gecontroleerd — geen bijwerkingen op de rest van het
  project.

## Nieuw ontwerp: "Tegeltje met tekst - Huisje kaasje wijntje"

Toegevoegd aan de drukwerkbestand-generator (`server/texttile.js`), op
dezelfde manier als eerder "Tussen de sterren" en "Blij met ons": een
sierlijk handschrift-lettertype zonder los aangeleverd fontbestand, dus
rechtstreeks als 20 vector-decoratie-vormen uit het aangeleverde
referentiebestand (`HUISJE_KAASJE_WIJNTJE_Tekengebied_1.pdf`) geëxtraheerd
— 3 tekstregels ("Huisje" / "Kaasje" / "Wijntje") plus een handgetekend
hartje aan het eind, dat (net als bij "Blij met ons" en "Tussen de
sterren") de vaste bordeaux-CMYK-kleur krijgt, ongeacht de gekozen
tegelkleur.

Herkenning: `/huisje\s*kaasje\s*wijntje/i` op de producttitel — getest tegen
de exacte titel uit de order ("Tegeltje met Tekst - Huisje kaasje wijntje.
– Rose / Zwart Metalen houder"), inclusief de kleur-extractie ("Rose" uit
de variant-titel).

**Getest**:
- De vectorvormen zijn geëxtraheerd door het referentiebestand zijn eigen
  PDF-tekenopdrachten (content stream) direct te ontleden (met `pikepdf`,
  inclusief het meenemen van de intern gebruikte transformatiematrices),
  i.p.v. via een font — dezelfde beproefde techniek als bij "Blij met
  ons".
- Het resultaat is gerenderd (`pdftoppm`) en pixel-voor-pixel vergeleken
  met een render van het originele referentiebestand: op wat
  anti-aliasing-ruis na (dunne contourlijntjes door verschillen in
  sub-pixel-rendering, geen enkel zichtbaar vorm- of positieverschil)
  volledig identiek.
- Ook getest via de daadwerkelijke `generateTegelTekstPdf`-functie zelf
  (i.p.v. alleen een losstaand testje) — zowel met een lichte tegelkleur
  (zwarte tekst, zichtbaar en correct) als met de daadwerkelijk bestelde
  kleur "Rose" (witte tekst — vólgens de al langer bestaande, met de
  opdrachtgever afgesproken regel dat alleen Wit/Beige zwarte tekst
  krijgen; op een wit test-canvas is witte tekst dan onzichtbaar, maar dat
  is verwacht gedrag, geen bug — op de daadwerkelijke roze tegel is dat wél
  gewoon zichtbaar).
- Volledige regressietest: alle 24 "Tegeltje met tekst"-ontwerpen (het
  nieuwe meegerekend) succesvol opnieuw gegenereerd zonder fouten, en alle
  servermodules syntax-gecontroleerd — geen bijwerkingen op de rest van
  het project.

## Bugfix: "Kon drukwerkbestanden niet downloaden: Failed to fetch" bij de bulk-download

Gemeld: op de live site gaf de knop "Download drukwerkbestanden" (voor
meerdere geselecteerde orders tegelijk) soms een JS-alert met deze
generieke netwerkfout, zonder duidelijke oorzaak. Bij onderzoek bleken er
**twee samenhangende bugs** in `/api/print-files/pdf-zip`
(`server/index.js`) en de gedeelde foto-ophaalfunctie
`fetchMetHerpogingen` (`server/pdf-shared.js`), allebei alleen zichtbaar
bij de BULK-download (niet bij het downloaden van 1 los drukwerkbestand):

1. **Geen timeout op foto-downloads.** `fetchMetHerpogingen` (gebruikt
   voor elke autopictura-/Posterly-/foto-tegel-link én in
   `lijntekeningframe.js`) gaf zijn opties gewoon door aan `axios.get()`
   zonder zelf een `timeout` te zetten — en axios wacht van zichzelf
   ONEINDIG lang als een verbinding wel tot stand komt maar de andere kant
   nooit antwoordt. Bij een bulk-download worden orders na elkaar
   afgehandeld: zodra ÉÉN foto-bron in de hele selectie vastliep, bleef de
   volledige download daar voor altijd op hangen — de browser kreeg
   minutenlang geen enkele byte, en uiteindelijk brak de verbinding af als
   de generieke `fetch`-fout "Failed to fetch". Bij een download van 1
   bestand viel dit zelden op (kleinere kans om precies die kapotte link te
   raken, en de al aanwezige page tijdens het wachten wordt niet als "hangt
   voor altijd" ervaren).
   **Fix**: een standaard-timeout van 15 seconden per poging toegevoegd
   (`STANDAARD_FETCH_TIMEOUT_MS` in `pdf-shared.js`, aanroeper kan 'm nog
   steeds overschrijven). Een timeout geeft axios' eigen `ECONNABORTED`-
   fout, die al in de bestaande "tijdelijke netwerkfout"-lijst stond, dus
   wordt automatisch (tot 3x) herhaald zoals al ontworpen — en faalt na de
   laatste poging gewoon binnen die ene order/item, met een keurige
   `FOUT-....txt` in de zip in plaats van de hele download te blokkeren.
2. **`archive.on('error', (err) => { throw err; })` kon het hele
   Node-proces laten crashen.** Dit archiver-event vuurt ASYNCHROON, buiten
   de call-stack van de try/catch rond de route — een `throw` daarbinnen
   wordt dus NIET door die try/catch opgevangen. Zonder een globale
   `process.on('uncaughtException')`-vangnet (niet aanwezig) crasht zo'n
   onopgevangen throw de HELE server, wat ALLE actieve verbindingen breekt
   (ook van andere gebruikers/andere pagina's) — wat zich voor de
   gebruiker ook als een netwerkfout ("Failed to fetch") voordoet, los van
   welke order het triggerde. Elders in hetzelfde bestand (de geplande
   zip-export) stond het al wél goed (`archive.on('error', reject)` binnen
   een Promise) — hier ontbrak dat.
   **Fix**: dezelfde soort veilige afhandeling toegevoegd — de fout loggen
   (`console.error`) en de response netjes beantwoorden/afbreken
   (`res.status(500)` als er nog geen headers verstuurd zijn, anders
   `res.destroy(err)`), zonder het proces te laten crashen. Ook de
   buitenste `catch` van de route doet nu hetzelfde bij een late fout
   (headers al verstuurd → `res.destroy()` i.p.v. een dubbele
   `res.status()`-aanroep die toch al een crash zou geven).

**Getest**:
- Een losstaand testje dat een server opzet die de verbinding accepteert
  maar nooit antwoordt: bevestigd dat `fetchMetHerpogingen` nu na de
  ingestelde timeout een `ECONNABORTED`-fout geeft (i.p.v. voor altijd te
  hangen), en dat een ECHTE meteen-mislukkende fout (`ECONNREFUSED`) nog
  steeds meteen doorgegeven wordt zonder op de volledige timeout te
  wachten.
- Een losstaand testje dat het oude `archive.on('error', (err) => {
  throw err; })`-patroon in een kindproces reproduceert: bevestigd dat dit
  het proces daadwerkelijk laat crashen (non-zero exit code) — en dat het
  nieuwe patroon, met exact dezelfde gesimuleerde archiver-fout, het proces
  in leven houdt en de fout netjes afhandelt.
- Volledige regressie: alle servermodules (`pdf-shared`, alle
  product-PDF-generators, `printnode`, `receiptHtml`, `shopify`, `db`,
  etc.) laden nog zonder fouten (`require` van elk bestand), en alle
  serverbestanden zijn syntax-gecontroleerd (`node -c`) — geen
  bijwerkingen op de rest van het project.
